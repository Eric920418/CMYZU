...
export default function CreateRankingPage() {
  ...
  const t = useTranslations('CreateRankingPage');
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      router.push('/login?redirect=/dashboard/rankings/create');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || '建立排名失敗');
      }
      ...
      setError(`建立排名項目時發生錯誤: ${errorMessage}`);
      console.error('建立排名錯誤:', err);
    } finally {
      setLoading(false);
    }
  };
        ...
        >
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              {t('title', { default: '新增排名' })}
            </h1>
            <p className="text-gray-600 mt-2">
              {t('description', { default: '添加學校新的權威排名資訊' })}
            </p>
          </div>
        </motion.div>
