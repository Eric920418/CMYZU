...
export default function EditAlumniPage({ params }: EditAlumniPageProps) {
  ...
  useEffect(() => {
    const fetchAlumni = async () => {
      ...
      try {
        const response = await dashboardAPI.alumni.get(params.id);
        if (response.success && response.data) {
          ...
          setFormData({
            ...
            nameEn: alumniData.nameEn || '',
            positionEn: alumniData.positionEn || '',
            descriptionEn: alumniData.descriptionEn || '',
            imageUrl: alumniData.imageUrl || '',
            achievements:
              alumniData.achievements?.length > 0
                ? alumniData.achievements
                : [''],
            achievementsEn:
              alumniData.achievementsEn?.length > 0
                ? alumniData.achievementsEn
                : [''],
            isActive: alumniData.isActive,
          });
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await dashboardAPI.alumni.update(params.id, {
      ...
      if (response.success) {
        ...
        setError(response.error || '更新校友失敗');
      }
    } catch (error) {
      console.error('更新校友錯誤:', error);
      setError('更新校友失敗');
    } finally {
  ...
  const handleDelete = async () => {
    if (!confirm('確定要刪除此校友嗎？此操作無法復原。')) {
      return;
    }

    try {
      const response = await dashboardAPI.alumni.delete(params.id);
      if (response.success) {
        router.push('/dashboard/alumni');
      } else {
        setError(response.error || '刪除校友失敗');
      }
    } catch (error) {
      console.error('刪除校友錯誤:', error);
      setError('刪除校友失敗');
    }
  };
      ...
      >
        ...
        <div className="mb-8">
          <nav className="flex mb-4" aria-label="Breadcrumb">
            <ol className="flex items-center space-x-4">
              ...
              <li className="flex items-center">
                ...
                <span className="text-gray-900 font-medium">編輯校友</span>
              </li>
            </ol>
          </nav>
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">編輯傑出校友</h1>
              <p className="text-sm text-gray-500 mt-1">修改校友資料和成就</p>
            </div>
            <button
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <label className="flex items-center">
                  ...
                  />
                  <span className="ml-2 text-sm text-gray-900">啟用狀態</span>
                </label>
              </div>
            </div>
          </div>
